...
function CreateSimForm() {
  ...
  const { user } = useAuth();
  const [simTypes, setSimTypes] = useState(null);
  ...
  useEffect(() => {
    async function getSimTypes() {
      ...
      let { data, error } = await supabaseClient
        .from("sim_types")
        .select("id, name, description, world_image, price, gestation");
    ...
    <>
      <Form onSubmit={handleCreateSim} className="form-widget">
        <Form.Group>
          ...
          <small>
            <p>
              * Different simulation types allow for different resources and
              collectables to be produced
            </p>
          </small>
        </Form.Group>
        <Form.Group>
          ...
          />
          <small>
            <p>* Name your new world whatever you like!</p>
          </small>
        </Form.Group>
      ...
      {selected ? (
        <>
          ...
          <hr />
          <h4>DROPS: {selected["gestation"]}mins</h4>
